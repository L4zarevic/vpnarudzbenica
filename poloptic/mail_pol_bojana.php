...
WHERE dobavljac="pol-bojana" ORDER BY lag_spec ASC');
...
$to="";
$stmt_email = $con->prepare('SELECT email FROM email_dobavljaca WHERE naziv_dobavljaca="pol-bojana"');
$stmt_email->execute();
$result_email = $stmt_email->get_result();
while ($row_email = $result_email->fetch_object()) {
  $to = $row_email->email;
}
//$to = "bojana.c@poloptic.com";
///////////////////////////////////
...
WHERE dobavljac="pol-bojana" ORDER BY lag_spec ASC');
  ...
  $message = "Narudzbenica - Pol Optic (Bojana) \n";
  ...
  $stmt2 = $con->prepare('INSERT INTO mojaopt_vpnarudzbenica.istorijat_pol (IDKorisnika,narudzba,datum,dobavljac) VALUES (?,?,?,"Poloptic - Bojana")');
  $stmt2->bind_param('iss', $idKorisnika, $schema_insert, date("Y-m-d"));
  $stmt2->execute();

  $stmt = $con->prepare('DELETE FROM `narudzbenica_pol` WHERE dobavljac = "pol-bojana"');
