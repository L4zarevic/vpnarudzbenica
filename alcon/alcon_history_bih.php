...
{
  $stmt = $con->prepare("SELECT COUNT(ID) AS brojRedova FROM istorijat_jj WHERE dobavljac = 'Johnson & Johnson - BiH' AND realizovana=''");
  $stmt->execute();
...
$result_count = mysqli_query($con, "SELECT COUNT(*) As total_records FROM istorijat_jj WHERE dobavljac = 'Johnson & Johnson - BiH'");
...
if (isset($_GET['realizovano']) && $_GET['realizovano'] != "") {
  if ($_GET['realizovano'] == "asc") {
    $stmt = $con->prepare("SELECT ID,IDKorisnika,datum,realizovana FROM istorijat_jj WHERE dobavljac = 'Johnson & Johnson - BiH' ORDER BY realizovana ASC LIMIT $offset, $total_records_per_page");
  } else {
                ...
                <table class="table table-bordered table-hover" id="table" data-toggle="table" width="100%" cellspacing="0">
                  <thead>
                    <tr>
                      ...
                      <th>Realizovano <a href="johnson_johnson_history_bih.php?realizovano=asc"><i class="fas fa-arrow-up" aria-hidden="true"></i></a>&nbsp;<a href="johnson_johnson_history_bih.php?realizovano=desc"><i class="fas fa-arrow-down" aria-hidden="true"></i></a></th>
                    </tr>
                  </thead>
                  <tbody>
                    <?php
