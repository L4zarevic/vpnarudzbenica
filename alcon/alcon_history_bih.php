...
{
  $stmt = $con->prepare("SELECT COUNT(ID) AS brojRedova FROM istorijat_alcon WHERE dobavljac = 'Alcon - BiH' AND realizovana=''");
  $stmt->execute();
...
$result_count = mysqli_query($con, "SELECT COUNT(*) As total_records FROM istorijat_alcon WHERE dobavljac = 'Alcon - BiH'");
...
if (isset($_GET['realizovano']) && $_GET['realizovano'] != "") {
  if ($_GET['realizovano'] == "asc") {
    $stmt = $con->prepare("SELECT ID,IDKorisnika,datum,realizovana FROM istorijat_alcon WHERE dobavljac = 'Alcon - BiH' ORDER BY realizovana ASC LIMIT $offset, $total_records_per_page");
  } else {
    $stmt = $con->prepare("SELECT ID,IDKorisnika,datum,realizovana FROM istorijat_alcon WHERE dobavljac = 'Alcon - BiH' ORDER BY realizovana DESC LIMIT $offset, $total_records_per_page");
  }
} else {
  $stmt = $con->prepare("SELECT ID,IDKorisnika,datum,realizovana FROM istorijat_alcon WHERE dobavljac = 'Alcon - BiH' ORDER BY ID DESC LIMIT $offset, $total_records_per_page");
}
$stmt->execute();
$result = $stmt->get_result();
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <h1 class="h3 mb-4 text-gray-800">Alcon BiH</h1>
                ...
                <table class="table table-bordered table-hover" id="table" data-toggle="table" width="100%" cellspacing="0">
                  <thead>
                    <tr>
                      ...
                      <th>Realizovano <a href="alcon_history_bih.php?realizovano=asc"><i class="fas fa-arrow-up" aria-hidden="true"></i></a>&nbsp;<a href="alcon_history_bih.php?realizovano=desc"><i class="fas fa-arrow-down" aria-hidden="true"></i></a></th>
                    </tr>
                  </thead>
                  <tbody>
                    <?php
