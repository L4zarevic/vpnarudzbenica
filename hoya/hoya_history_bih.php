...
{
  $stmt = $con->prepare("SELECT COUNT(ID) AS brojRedova FROM istorijat_hoya WHERE dobavljac = 'Hoya - BiH' AND realizovana=''");
  $stmt->execute();
...
$result_count = mysqli_query($con, "SELECT COUNT(*) As total_records FROM istorijat_hoya WHERE dobavljac = 'Hoya - BiH'");
...
if (isset($_GET['realizovano']) && $_GET['realizovano'] != "") {
  if ($_GET['realizovano'] == "asc") {
    $stmt = $con->prepare("SELECT ID,IDKorisnika,datum,realizovana FROM istorijat_hoya WHERE dobavljac = 'Hoya - BiH' ORDER BY realizovana ASC LIMIT $offset, $total_records_per_page");
  } else {
    $stmt = $con->prepare("SELECT ID,IDKorisnika,datum,realizovana FROM istorijat_hoya WHERE dobavljac = 'Hoya - BiH' ORDER BY realizovana DESC LIMIT $offset, $total_records_per_page");
  }
} else {
  $stmt = $con->prepare("SELECT ID,IDKorisnika,datum,realizovana FROM istorijat_hoya WHERE dobavljac = 'Hoya - BiH' ORDER BY ID DESC LIMIT $offset, $total_records_per_page");
}
$stmt->execute();
$result = $stmt->get_result();
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <h1 class="h3 mb-4 text-gray-800">Hoya BiH</h1>
