...
$to="";
$stmt_email = $con->prepare('SELECT email FROM email_dobavljaca WHERE naziv_dobavljaca="hoya-srbija"');
$stmt_email->execute();
$result_email = $stmt_email->get_result();
while ($row_email = $result_email->fetch_object()) {
  $to = $row_email->email;
}
//$to = "office@mobioptika.rs";
///////////////////////////////////
...
if (mail($to, $subject, $body, $headers)) {
  ...
  $message = "Narudzbenica - Hoya Srbija \n";
  ...
  $stmt2 = $con->prepare('INSERT INTO mojaopt_vpnarudzbenica.istorijat_hoya (IDKorisnika,narudzba,datum,dobavljac) VALUES (?,?,?,"Hoya - Srbija")');
  $stmt2->bind_param('iss', $idKorisnika, $schema_insert, date("Y-m-d"));
  $stmt2->execute();

  $stmt = $con->prepare('DELETE FROM `narudzbenica_hoya` WHERE dobavljac = "hoya-srbija"');
