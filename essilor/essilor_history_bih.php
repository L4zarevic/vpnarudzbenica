...
{
  $stmt = $con->prepare("SELECT COUNT(ID) AS brojRedova FROM istorijat_essilor WHERE dobavljac = 'Poloptic - Beograd' AND realizovana=''");
  $stmt->execute();
...
$result_count = mysqli_query($con, "SELECT COUNT(*) As total_records FROM istorijat_essilor WHERE dobavljac = 'Poloptic - Beograd'");
...
if (isset($_GET['realizovano']) && $_GET['realizovano'] != "") {
  if ($_GET['realizovano'] == "asc") {
    $stmt = $con->prepare("SELECT ID,IDKorisnika,datum,realizovana FROM istorijat_essilor WHERE dobavljac = 'Poloptic - Beograd' ORDER BY realizovana ASC LIMIT $offset, $total_records_per_page");
  } else {
    $stmt = $con->prepare("SELECT ID,IDKorisnika,datum,realizovana FROM istorijat_essilor WHERE dobavljac = 'Poloptic - Beograd' ORDER BY realizovana DESC LIMIT $offset, $total_records_per_page");
  }
} else {
  $stmt = $con->prepare("SELECT ID,IDKorisnika,datum,realizovana FROM istorijat_essilor WHERE dobavljac = 'Poloptic - Beograd' ORDER BY ID DESC LIMIT $offset, $total_records_per_page");
}
$stmt->execute();
$result = $stmt->get_result();
