...
$result_count = mysqli_query($con, "SELECT COUNT(*) As total_records FROM istorijat_essilor WHERE dobavljac = 'Essilor - Srbija'");
...
if (isset($_GET['realizovano']) && $_GET['realizovano'] != "") {
  if ($_GET['realizovano'] == "asc") {
    $stmt = $con->prepare("SELECT ID,IDKorisnika,datum,realizovana FROM istorijat_essilor WHERE dobavljac = 'Essilor - Srbija' ORDER BY realizovana ASC LIMIT $offset, $total_records_per_page");
  } else {
    $stmt = $con->prepare("SELECT ID,IDKorisnika,datum,realizovana FROM istorijat_essilor WHERE dobavljac = 'Essilor - Srbija' ORDER BY realizovana DESC LIMIT $offset, $total_records_per_page");
  }
} else {
  $stmt = $con->prepare("SELECT ID,IDKorisnika,datum,realizovana FROM istorijat_essilor WHERE dobavljac = 'Essilor - Srbija' ORDER BY ID DESC LIMIT $offset, $total_records_per_page");
}
$stmt->execute();
$result = $stmt->get_result();
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <h1 class="h3 mb-4 text-gray-800">Essilor Srbija</h1>
                ...
                <table class="table table-bordered table-hover" id="dataTable" data-toggle="table" width="100%" cellspacing="0">
                  <thead>
                    <tr>
                      ...
                      <th>Datum</th>
                      <th>Naručio</th>
                      <th>Realizovano <a href="essilor_history_srb.php?realizovano=asc"><i class="fas fa-arrow-up" aria-hidden="true"></i></a>&nbsp;<a href="essilor_history_srb.php?realizovano=desc"><i class="fas fa-arrow-down" aria-hidden="true"></i></a></th>
                    </tr>
                  </thead>
                  <tbody>
                    <?php
