...
if (isset($_GET['realizovano']) && $_GET['realizovano'] != "") {
    $stmt1 = $con->prepare('UPDATE istorijat_essilor SET realizovana="&check;", IDKorisnika_realizovao=? WHERE ID=?');
...
if (isset($_REQUEST['napomena'])) {
    ...
    $stmt2 = $con->prepare('UPDATE istorijat_essilor SET napomena=? WHERE ID=?');
    $stmt2->bind_param('si', $napomena, $id_stavke);
    $stmt2->execute();
}

$stmt = $con->prepare('SELECT * FROM istorijat_essilor WHERE ID=?');
...
if ($realizovana == "&check;") {
    ...
    echo "</table></br></br> <center><button type='button' class='btn btn-success' onClick=window.location.href='\../essilor/orders_document_preview.php?id=$id_stavke&realizovano=true'>Potvrdi prispeće ove narudžbe</button></center>";
}
